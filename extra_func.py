...
def visualize_analytical_solution(cur_type, costs, tt,E,r,gamma,T_prime,B):
...
def visualize_numerical_solution(costs, tt, values, costs_grid, g,E,B,M,T_prime):
    ...
    if g == g_eu_call:
        ax.plot(np.ones(M+1) * T_prime, costs_grid, g(costs_grid, E),linewidth = 4,color='blue')
    if g == g_eu_put:
        ax.plot(np.ones(M+1) * T_prime, costs_grid, g(costs_grid, E),linewidth = 4,color='blue')
    if g == g_cash_or_nothing:
        ax.plot(np.ones(M+1) * T_prime, costs_grid, g(costs_grid, E,B),linewidth = 4,color='blue')
